ConstellaWallet version 2.0.0 is now available from:
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/proyecto-constella/constellawallet/issues>
...
updates will be released anymore. Without security updates, using a constella
...
ECDSA signatures inside Constella transactions now use validation using
...
Previous versions of ConstellaWallet had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of ConstellaWallet was to raise the default minimum
relay fee.

ConstellaWallet 0.13.2 will have a strict maximum size on the mempool. The
...
ConstellaWallet 0.13.2 also introduces new default policy limits on the length and
...
This allows running constellad without having to do any manual configuration.
...
ConstellaWallet has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. ConstellaWallet will relay transactions with
...
entirely in ConstellaWallet 0.13, and it is at this time undecided whether the
...
ConstellaWallet has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
ConstellaWallet automatically creates a hidden service to listen on, without
manual configuration. ConstellaWallet will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if ConstellaWallet is listening, and
...
Constellad can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Constella
Core to use floating fees. By default, ConstellaWallet will use floating
fees.
...
(default: `0.0002` XCL/kB).

At all times, ConstellaWallet will cap fees at `-maxtxfee=<x>` (default:
0.10) XCL.
Furthermore, ConstellaWallet will never create transactions paying less than
...
`constella.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /constellarpc>
        ...
        # optional bypass constellad rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< constellarpc:<password>
    </Location>
...
Restarting constellad will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in constella.conf
...
constella-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/constella-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The ConstellaWallet code base has
...
- `constella-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `constella-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Constella scripting system that in combination with
...
Constella transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of ConstellaWallet.
...
The two phases are distinct in the ConstellaWallet GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Constella wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Constella.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Constella block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of constellas they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of constellas being spent is exactly
...
- **Increased security for multisig:** Constella addresses (both P2PKH addresses
  ...
  believed to be provided by Constella's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Constella's current ledger (called the UTXO set),
  ...
  note that ConstellaWallet does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Constella users to individually opt-in to almost any change in the Constella
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and ConstellaWallet contributors that may
...
segwit's start date of 1 January 2017 miners can update the Constella
client to ConstellaWallet 0.13.2 to signal for segwit support. When a
...
Since ConstellaWallet 0.10.0, nodes have defaulted to only relaying and
...
- `constella-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `constella-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
