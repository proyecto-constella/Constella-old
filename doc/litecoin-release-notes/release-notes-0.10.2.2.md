ConstellaWallet version 0.10.2.2 is now available from:
...
installer (on Windows) or just copy over /Applications/ConstellaWallet (on Mac) or
constellad/constellawallet (on Linux).
...
backwards-compatible with pre-0.10 versions of ConstellaWallet or other software:
...
- ConstellaWallet v0.10.2.2 is protocol version 70003 (instead of 70002)

Relay:
- ConstellaWallet rounds transaction size up to the nearest 1000 bytes before calculating fees.  This size rounding behavior is to mimic fee calculation of Litecoin v0.6 and v0.8.
...
- constellad no longer sends RPC commands.  You must use the separate constella-cli command line utility.
