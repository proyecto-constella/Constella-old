...
ConstellaWallet is a multithreaded application, and deadlocks or other multithreading bugs
...
$ valgrind -v --leak-check=full src/constellad -printtoconsole
```
...
However, in open source software such as ConstellaWallet, where everyone uses
...
pay attention to for reviewers of ConstellaWallet code.

General ConstellaWallet
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: ConstellaWallet uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of ConstellaWallet, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against ConstellaWallet so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `constella-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `constella-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of ConstellaWallet, the wallet was always
