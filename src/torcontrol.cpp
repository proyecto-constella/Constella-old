 ...
 * the server reply formats for PROTOCOLINFO (S3.21) and AUTHXCLLLENGE (S3.24).
 */
 ...
 * Grammar is implicitly defined in https://spec.torproject.org/control-spec by
 * the server reply formats for PROTOCOLINFO (S3.21), AUTHXCLLLENGE (S3.24),
...
private:
    ...
    /** Callback for AUTHXCLLLENGE result */
 ...
 *
 *    After a controller sends a successful AUTHXCLLLENGE command, the
...
void TorController::authchallenge_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        if (l.first == "AUTHXCLLLENGE") {
            std::map<std::string,std::string> m = ParseTorReplyMapping(l.second);
            if (m.empty()) {
                LogPrintf("tor: Error parsing AUTHXCLLLENGE parameters: %s\n", SanitizeString(l.second));
                return;
            }
            std::vector<uint8_t> serverHash = ParseHex(m["SERVERHASH"]);
            std::vector<uint8_t> serverNonce = ParseHex(m["SERVERNONCE"]);
            LogPrint(BCLog::TOR, "tor: AUTHXCLLLENGE ServerHash %s ServerNonce %s\n", HexStr(serverHash), HexStr(serverNonce));
            ...
            LogPrintf("tor: Invalid reply to AUTHXCLLLENGE\n");
        }
    } else {
    ...
    if (reply.code == 250) {
        ...
        if (!torpassword.empty()) {
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                ...
                _conn.Command("AUTHXCLLLENGE SAFECOOKIE " + HexStr(clientNonce), boost::bind(&TorController::authchallenge_cb, this, _1, _2));
            } else {
