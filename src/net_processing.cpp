...
struct CNodeState {
    ...
    /** State used to enforce XCLIN_SYNC_TIMEOUT
      * Only in effect for outbound, non-manual connections, with
      * m_protect == false
      * Algorithm: if a peer's best known block has less work than our tip,
      * set a timeout XCLIN_SYNC_TIMEOUT seconds in the future:
...

    {
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            if (!chainActive.Contains(pindexWalk)) {
                ...
                if (vGetData.size() > 0) {
                    if (nodestate->fSupportsDesiredCmpctVersion && vGetData.size() == 1 && mapBlocksInFlight.size() == 1 && pindexLast->pprev->IsValid(BLOCK_VALID_XCLIN)) {
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (nServices & ((1 << 7) | (1 << 5))) {
            if (GetTime() < 1533096000) {
                // Immediately disconnect peers that use service bits 6 or 8 until August 1st, 2019-2020
...
void PeerLogicValidation::ConsiderEviction(CNode *pto, int64_t time_in_seconds)
{
    ...
    if (!state.m_chain_sync.m_protect && IsOutboundDisconnectionCandidate(pto) && state.fSyncStarted) {
        ...
        // XCLIN_SYNC_TIMEOUT + HEADERS_RESPONSE_TIME seconds (note: if
        ...
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= chainActive.Tip()->nChainWork) {
            ...
            state.m_chain_sync.m_timeout = time_in_seconds + XCLIN_SYNC_TIMEOUT;
...
bool PeerLogicValidation::SendMessages(CNode* pto, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (pto->nVersion >= FEEFILTER_VERSION && gArgs.GetBoolArg("-feefilter", DEFAULT_FEEFILTER) &&
            ...
            // If the fee filter has changed substantially and it's still more than MAX_FEEFILTER_XCLNGE_DELAY
            // until scheduled broadcast, then move the broadcast to within MAX_FEEFILTER_XCLNGE_DELAY.
            else if (timeNow + MAX_FEEFILTER_XCLNGE_DELAY * 1000000 < pto->nextSendTimeFeeFilter &&
                     (currentFilter < 3 * pto->lastSentFeeFilter / 4 || currentFilter > 4 * pto->lastSentFeeFilter / 3)) {
                pto->nextSendTimeFeeFilter = timeNow + GetRandInt(MAX_FEEFILTER_XCLNGE_DELAY) * 1000000;
            }
        }
    }
    return true;
