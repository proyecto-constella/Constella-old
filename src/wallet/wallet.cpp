...
bool bSpendZeroConfChange = DEFAULT_SPEND_ZEROCONF_XCLNGE;
...
bool CWallet::SetHDMasterKey(const CPubKey& pubkey)
{
    ...
    // as a hdchain object
    CHDChain newHdChain;
    newHdChain.nVersion = CanSupportFeature(FEATURE_HD_SPLIT) ? CHDChain::VERSION_HD_XCLIN_SPLIT : CHDChain::VERSION_HD_BASE;
                                 ...
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet) const
{
    ...
    for (const COutput &output : vCoins)
    {
        ...
        else if (coin.txout.nValue < nTargetValue + MIN_XCLNGE)
        {
    ...
    if (nBest != nTargetValue && nTotalLower >= nTargetValue + MIN_XCLNGE)
        ApproximateBestSubset(vValue, nTotalLower, nTargetValue + MIN_XCLNGE, vfBest, nBest);

    // If we have a bigger coin and (either the stochastic approximation didn't find a good solution,
    //                                   or the next bigger coin is closer), return the bigger coin
    if (coinLowestLarger &&
        ((nBest != nTargetValue && nBest < nTargetValue + MIN_XCLNGE) || coinLowestLarger->txout.nValue <= nBest))
    {
        setCoinsRet.insert(coinLowestLarger.get());
        nValueRet += coinLowestLarger->txout.nValue;
    }
...
bool CWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl) const
{
    ...
    bool fRejectLongChains = gArgs.GetBoolArg("-walletrejectlongchains", DEFAULT_WALLET_REJECT_LONG_XCLINS);
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
                    ...
                    if (change_position->nValue >= MIN_FINAL_XCLNGE + additionalFeeNeeded) {
    ...
    if (gArgs.GetBoolArg("-walletrejectlongchains", DEFAULT_WALLET_REJECT_LONG_XCLINS)) {
