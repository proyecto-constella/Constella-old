...
const std::string strMessageMagic = "Constella Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // 60000 primer bloque, 10 hacia adelante, variable desde el 2834 hasta el 60000, luego fijo a 10 denuevo
    if (nHeight == 1)
        nSubsidy = 60000 * COIN;

    if (nHeight > 2834 && nHeight <= 60000) // En realidad partio desde el 2835.
        nSubsidy = (60000/nHeight + 10) * COIN;
...
void CChainState::CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    CBlockIndex* pindexFirstNotChainValid = nullptr; // Oldest ancestor of pindex which does not have BLOCK_VALID_XCLIN (regardless of being valid or not).
    ...
    while (pindex != nullptr) {
        ...
        if (pindex->pprev != nullptr && pindexFirstNotChainValid == nullptr && (pindex->nStatus & BLOCK_VALID_MASK) < BLOCK_VALID_XCLIN) pindexFirstNotChainValid = pindex;
        ...
        if ((pindex->nStatus & BLOCK_VALID_MASK) >= BLOCK_VALID_XCLIN) assert(pindexFirstNotChainValid == nullptr); // XCLIN valid implies all parents are XCLIN valid
